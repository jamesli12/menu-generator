...
const InputPage = () => {
  ...
  // Function to handle changes to food item properties
    ...
    if (field === "picture") {
      if (value.target.files) {
        newFoodItems[index] = {
          ...
          pictureUrl: URL.createObjectURL(value.target.files[0]),
        };
      }
    } else {
  ...
  // Function to handle changes to the restaurant logo
  ...
  const handleSubmit = async (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    foodItems.forEach((item, index) => {
      ...
      formData.append(
        `foodItems[${index}].dietaryRestrictions`,
        item.dietaryRestrictions
      );
      formData.append(
        `foodItems[${index}].vegetarian`,
    ...
    try {
      if (localStorage.token) {
        const response = await fetch(
          import.meta.env["VITE_BACKEND_URI"] + "/submit",
          {
            method: "POST",
            headers: {
  ...
  return (
    <div className="flex flex-col items-center justify-center bg-gray-100">
      <Card
        shadow-sm
        className="min-w-[400px] mx-auto p-4 items-center mb-5 mt-5"
      >
      ...
      <h2 className="text-center text-2xl font-bold mb-5">Menu Items</h2>
                  ...
                  <>
                    ...
                    <ModalBody>
                      <Input
                        type="file"
                        onChange={(e) =>
                          handleFoodItemChange(index, "picture", e)
                        }
                        accept="image/*"
                        style={{ padding: 10, borderColor: "#0070f3" }}
                      />
